...
const inkPlayerPackage = JSON.parse(readFileSync(inkPlayerPackageJson, 'utf-8'));
...
if (templatePackage.dependencies && templatePackage.dependencies['@lamppost/ink-player']) {
    const oldVersion = templatePackage.dependencies['@lamppost/ink-player'];
    templatePackage.dependencies['@lamppost/ink-player'] = `^${inkPlayerVersion}`;
    console.log(`Updated @lamppost/ink-player from ${oldVersion} to ^${inkPlayerVersion}`);
} else {
    console.error('Error: @lamppost/ink-player dependency not found in template package.json');
    process.exit(1);
}

// Write updated template package.json
writeFileSync(templatePackageJson, JSON.stringify(templatePackage, null, 4) + '\n');
